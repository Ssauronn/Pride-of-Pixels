...
///@param	{real}		x_		The x value to check
///@param	{real}		y_		The y value to check
///@param	{string}	Type	optional - objectType
///@description					Detects the closest enemy object out of any potential enemy
///								objects around.

function detect_closest_enemy_object(x_, y_) {
	/*
			...
			if instance_exists(instance_at_location_) {
				...
				if (instance_at_location_.objectClassification != "Resource") && (instance_at_location_.objectType != "Farm") && (instance_at_location_.objectType != "Thicket") && (instance_at_location_.objectType != "Mine") {
					...
					if (instance_at_location_.objectVisibleTeam != objectRealTeam) && (instance_at_location_.objectRealTeam != objectRealTeam) {
						if line_of_sight_exists_to_target(floor(x_ / 16) * 16, floor(y_ / 16) * 16, floor(instance_at_location_.x / 16) * 16, floor(instance_at_location_.y / 16) * 16) {
							if (!target_specified_) || (instance_at_location_.objectType == target_specified_) {
								...
								else if (point_distance(x_, y_, instance_at_location_.x, instance_at_location_.y)) < (point_distance(x_, y_, closest_object_.x, closest_object_.y)) {
									closest_object_ = instance_at_location_.id;
								}
							}
						}
